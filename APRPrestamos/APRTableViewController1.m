...
{
    Objeto *o = [self.frController objectAtIndexPath:indexPath];
    cell.textLabel.text = o.nombre;
    NSString *string = [NSString stringWithFormat:@"%@", o.estado, nil];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if([segue.identifier isEqualToString:@"showDetail1"]){
         ...
         APRDetailController1 * destino = segue.destinationViewController;
...
- (NSFetchedResultsController *)frController
{
    if (_frController == nil) {
        ...
        NSEntityDescription *entidad = [NSEntityDescription entityForName:@"Objeto" inManagedObjectContext:self.contexto];
...
shouldReloadTableForSearchString:(NSString *)searchString{
    ...
    fetchRequest.predicate = [NSPredicate predicateWithFormat:@"nombre contains[cd] %@ or estado contains[cd] %@", searchString, searchString];
    
    fetchRequest.entity = [NSEntityDescription entityForName:@"Objeto" inManagedObjectContext:self.contexto];
...
- (IBAction)addObjeto:(id)sender {
    ...
    APRNewController1 * c = [stb instantiateViewControllerWithIdentifier:@"add1"];
    c.delegado = self;
